...
void ImGuiFullscreen::DrawNotifications(ImVec2& position, float spacing)
{
	...
	for (u32 index = 0; index < static_cast<u32>(s_notifications.size());)
	{
		...
		// ImGui v1.92 AddRect signature: (p_min, p_max, col, rounding, flags, thickness).
		// Pre-1.82 order was (..., rounding_corners, thickness); swapping causes
		// LayoutScale(1.0f) to be cast to ImDrawFlags with low-nibble bits set
		// → FixRectCornerFlags assertion. flags before thickness.
