...
{
	// @@IOP_BLOCKLINK@@ Unpatch any inbound direct-B links whose target is in the cleared
	// range, and drop link records for the cleared blocks, BEFORE the slots are zeroed and
	// the host code is recycled — so no predecessor can branch into a stale block. Uses the
	// same recPtrToBlock-slot keys as the loop below, so it folds mirrors identically.
	if (s_iopBlockLinkEnabled)
		iopInvalidateLinks(Addr, Size);
