...
// Host-MMU fastmem backpatch (@@MAC_FASTMEM_BACKPATCH@@). Called from the shared
// vtlb_BackpatchLoadStore (vtlb.cpp) when a single fastmem Ldr/Str at `code_address`
// faults on a handler/MMIO/unmapped page. We carve a thunk that redoes the access via
// the slow vtlb_mem* helper (using the recorded address/data registers), resumes at the
// instruction after the fault, then overwrite the faulting Ldr/Str with a branch to the
// thunk so subsequent executions skip straight to the slow path (no re-fault). mac has no
// persistent RCYCLE, so — unlike the stock thunk — there is no cycle flush/reload.
//
// address_register / data_register are ARM64 register codes; they can be EE guest-GPR
// cache regs (x20/x22-x27), not just scratch, so the moves below preserve them where
// needed. RESTATEPTR(x19)/REVTLBPTR(x21)/RFASTMEMBASE(x28) are callee-saved and survive
// the C call. Only integer GPR loads/stores emit fastmem so far (FPR + 128-bit quads
// still use the inline-vmap path), so those are asserted out here.
void vtlb_DynBackpatchLoadStore(uptr code_address, u32 code_size, u32 guest_pc, u32 guest_addr, u32 gpr_bitmask, u32 fpr_bitmask, u8 address_register, u8 data_register, u8 size_in_bits, bool is_signed, bool is_load, bool is_fpr)
{
	...
	// No cycle reload (mac uses a memory-based cycle model). Resume at the instruction
	// after the faulting access.
...
// Global symbol the shared vtlb fault path (vtlb.cpp: vtlb_BackpatchLoadStore) binds to via
// an unqualified call. On Android only the mac EE rec runs, so forward the backpatch to its
// thunk. The stock arm64/aVTLB.cpp global definition is #if 0'd out. @@MAC_FASTMEM_BACKPATCH@@
...
{
	pcsx2_macrec::vtlb_DynBackpatchLoadStore(code_address, code_size, guest_pc, guest_addr,
		gpr_bitmask, fpr_bitmask, address_register, data_register, size_in_bits, is_signed,
		is_load, is_fpr);
}
