	...
	if (VMManager::GetState() != VMState::Paused)
	{
		if (s_last_update_timer.GetTimeNanoseconds() >= UPDATE_INTERVAL)
		{
			...
			if (GSConfig.OsdShowVersion)
				s_speed_line.append_format("{}ARMSX2 {}", s_speed_line.empty() ? "" : " | ", BuildVersion::GitRev);
			...
			{
				// Auto-scale to K/M/B so the counter stays readable regardless of
				// scene complexity. VSI = vertex-shader invocations, PSI =
				// pixel/fragment-shader invocations (Vulkan pipeline stats; the GL
				// backend reports 0 since GLES has no pipeline_statistics_query).
