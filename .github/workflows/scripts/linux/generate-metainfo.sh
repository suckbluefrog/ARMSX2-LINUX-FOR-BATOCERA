...
OUTFILE=$1
GIT_DATE=$(git log -1 --pretty=%cd --date=iso8601-strict)
GIT_VERSION=$(git tag --points-at HEAD | head -1)
...
sed -i -e "s|@GIT_VERSION@|${GIT_VERSION}|g" "${OUTFILE}"
sed -i -e "s|@GIT_DATE@|${GIT_DATE}|g" "${OUTFILE}"
sed -i -e "s|@GIT_HASH@|${GIT_HASH}|g" "${OUTFILE}"
